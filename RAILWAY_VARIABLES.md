...
```env
CLOSING_RECOVERY_POSITIONS=042700|한미반도체|2026-07-15|2026-07-16,004310|현대약품|2026-07-16|2026-07-30
```
...
| Variable | 설명 |
|----------|------|
| `CLOSING_ACCOUNT_SYNC` | `true`면 재배포 후 계좌 잔고에서 미추적 종목을 종가베팅으로 자동 복원 (블랙리스트·제외 목록 제외) |
| `CLOSING_HOLD_EXCLUDE` | 종가 자동복원·경고에서 제외할 종목 (쉼표 구분, 코드 또는 종목명) |
...
| `UPPER_MA5_GAP_MAX` | `15.0` | 상단매매 MA5 이격 상한% (과열 추격 완화) |
| `BREAKOUT_HIGH20_MAX_PCT` | `3.0` | 돌파-안전: 20일고가 대비 최대 +% |
| `BREAKOUT_MA5_GAP_MAX` | `10.0` | 돌파-안전: MA5 이격 상한% |
| `BREAKOUT_MOMENTUM_ENABLED` | `true` | 돌파-모멘텀(장초 소액) ON |
| `BREAKOUT_MOMENTUM_START` | `09:10` | 모멘텀 허용 시작 |
| `BREAKOUT_MOMENTUM_END` | `10:30` | 모멘텀 허용 종료 |
| `BREAKOUT_MOMENTUM_BUY_RATIO` | `0.5` | 모멘텀 매수금 = 1회한도×비율 |
| `BREAKOUT_MOMENTUM_HIGH20_MAX_PCT` | `8.0` | 모멘텀: 20일고가 대비 최대 +% |
| `BREAKOUT_MOMENTUM_MA5_GAP_MAX` | `20.0` | 모멘텀: MA5 이격 상한% |
| `BREAKOUT_MOMENTUM_5MIN_VOL` | `1.5` | 모멘텀: 5분봉 거래량 배수 |
...
## 🟢 강세주 급락 V — 시뮬만 (`ENABLE_STRONG_V_SIM=true`)

실제 주문 없음. **강세주(+2%+) 장중 급락 후 V반등** 패턴을 가상 매매로 검증합니다.
(SK하이닉스형 — 기존 V자반등보다 필터 완화)

| Variable | 기본값 | 설명 |
|----------|--------|------|
| `ENABLE_STRONG_V_SIM` | `false` | 강세V 시뮬 ON |
| `STRONG_V_SCAN_START` | `09:00` | 넓은 스캔 시작 |
| `STRONG_V_ENTRY_START` | `09:15` | 후보 1분 추적 → 매수 시작 |
| `STRONG_V_ENTRY_END` | `14:30` | 진입 스캔 종료 (오후 급락 V 포함) |
| `STRONG_V_POLL_INTERVAL` | `5` | 기본 스캔 주기(분) |
| `STRONG_V_FAST_SCAN_INTERVAL` | `2` | 09:00~10:00 스캔 주기(분) |
| `STRONG_V_FOCUS_INTERVAL` | `1` | 후보·보유 추적 주기(분) |
| `STRONG_V_MAX_BELOW_PREV` | `5.0` | 전일종가 대비 최대 -5% (V자 완화) |
| `STRONG_V_MA5_TOLERANCE` | `3.0` | MA5 아래 N%까지 허용 (MA5 완화) |
| `STRONG_V_MIN_PREV_RATE` | `2.0` | 전일比 최소 +2% |
| `STRONG_V_MIN_DROP` | `3.0` | 시가 대비 최소 -3% |
| `STRONG_V_SIM_AMOUNT` | `500000` | 가상 1회 매수금 |
| `STRONG_V_MAX_API_CALLS` | `20` | 스캔당 API 상한 |
| `STRONG_V_TIME_EXIT` | `14:20` | 시간 청산 (익절 중) |
| `STRONG_V_TAKE_PROFIT` | `3.0` | 트레일링 익절 기준 +3% |
| `STRONG_V_TRAILING_STOP` | `1.0` | 고점 대비 하락 % |
| `STRONG_V_STOP_LOSS` | `2.0` | 손절 -2% |
| `STRONG_V_MA_EXIT` | `false` | 5분봉 MA 저항 익절 (기본 OFF → 트레일링만) |

> 기존 `ENABLE_V_REVERSAL` 실전 전략과 별개. 시뮬만 동작하며 하루 1회 진입.
> **주기**: 기본 5분 스캔 → 09~10시 2분 → 후보 1종목 포착 후 해당 종목만 1분 추적·매수.
> **청산**: 손절 −2% / 트레일링(+3% 후 고점 −1%) / 14:20 시간청산. MA 저항 익절은 기본 끔.
...
세력봉(거래대금 500억+, 상한가 제외) 당일 K1 훼손 + 당일 양봉 시 가상 매수. 실제 주문 없음.
기본은 **모니터 구간 중 K1 훼손 즉시** 매수.

| Variable | 기본값 | 설명 |
|----------|--------|------|
| `ENABLE_K1_PLUS_SIM` | `false` | K1플러스 시뮬 ON |
| `K1_PLUS_IMMEDIATE_ON_BREACH` | `true` | true=모니터 중 훼손 즉시 / false=종가창만 |
| `K1_PLUS_ENTRY_START` | `14:20` | 종가창 시작 (`IMMEDIATE=false`일 때) |
| `K1_PLUS_ENTRY_END` | `14:50` | 종가창 종료 |
| `K1_PLUS_MONITOR_START` | `09:10` | 추적·즉시매수 시작 |
| `K1_PLUS_MONITOR_END` | `14:50` | 추적·즉시매수 종료 |
...
| `K1_PLUS_STOP_LOSS_PCT` | `5.0` | 매수가 대비 고정 손절 % (A) |
| `K1_PLUS_MAX_RISK_TO_LOW_PCT` | `8.0` | 저점까지 거리 >N%면 진입 스킵 (C) |
...
- `ENABLE_INTRADAY_AI`, `ENABLE_CRASH_BOUNCE`, `ENABLE_V_REVERSAL`, `ENABLE_STRONG_V_SIM`, `ENABLE_UL_REBOUND`, `ENABLE_K2_SIM`, `ENABLE_K1_PLUS_SIM`, `ENABLE_K2_PLUS_SIM`
