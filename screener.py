...
기술 조건 (종산 매매법, 2순위 완화):
  ...
  - 돌파매매: 20일 최고가 돌파, 5일선 위, 거래량 120%↑, 윗꼬리 35%↓
    (장중) 안전=고가근접+MA5이격 / 모멘텀=장초 소액 추격, 둘 다면 안전 우선
...
# 돌파매매 — 안전(1+2): 20일고가 근접 + MA5 이격 제한
BREAKOUT_HIGH20_MAX_PCT = float(os.getenv("BREAKOUT_HIGH20_MAX_PCT", "3.0"))
BREAKOUT_MA5_GAP_MAX = float(os.getenv("BREAKOUT_MA5_GAP_MAX", "10.0"))
# 돌파매매 — 모멘텀(장초 소액 추격 허용)
BREAKOUT_MOMENTUM_ENABLED = os.getenv("BREAKOUT_MOMENTUM_ENABLED", "true").lower() == "true"
BREAKOUT_MOMENTUM_BUY_RATIO = float(os.getenv("BREAKOUT_MOMENTUM_BUY_RATIO", "0.5"))
BREAKOUT_MOMENTUM_HIGH20_MAX_PCT = float(os.getenv("BREAKOUT_MOMENTUM_HIGH20_MAX_PCT", "8.0"))
BREAKOUT_MOMENTUM_MA5_GAP_MAX = float(os.getenv("BREAKOUT_MOMENTUM_MA5_GAP_MAX", "20.0"))
BREAKOUT_MOMENTUM_5MIN_VOL = float(os.getenv("BREAKOUT_MOMENTUM_5MIN_VOL", "1.5"))
...
BREAKOUT_MOMENTUM_START_MIN = _parse_hhmm(
    os.getenv("BREAKOUT_MOMENTUM_START", "09:10"), 9, 10,
)
BREAKOUT_MOMENTUM_END_MIN = _parse_hhmm(
    os.getenv("BREAKOUT_MOMENTUM_END", "10:30"), 10, 30,
)
