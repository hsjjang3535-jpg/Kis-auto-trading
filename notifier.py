...
def notify_k1_plus_alert(
    ...
    label = _K1_PLUS_LABELS.get(alert_type, alert_type)
    lines = [
        f"💠 <b>K1플러스 [{label}]</b>",
        f"종목: {name} ({code})",
        f"현재가: {current:,}원" if current > 0 else "",
        f"세력봉일: {entry.get('power_date', entry.get('scan_date', '-'))}",
        f"K1 {entry.get('k1', 0):,} | K2 {entry.get('k2', 0):,} | "
        f"고점 {entry.get('fib_high', 0):,} | 저점 {entry.get('fib_low', 0):,}",
        f"💡 {message}",
        "⚠️ 시뮬만 — 실제 주문 없음",
...
def notify_k1_plus_sim_buy(
    name: str, code: str, quantity: int, price: int, k1: int, reason: str,
) -> None:
    send(
        f"💠🟢 <b>[K1플러스 시뮬] 매수</b>\n"
        f"종목: {name} ({code})\n"
        ...
        f"K1선: {k1:,}원\n"
        f"사유: {reason}\n"
        f"⚠️ 시뮬만 — 실제 주문 없음 / 4일차 청산"
    )


def notify_k1_plus_sim_sell(
    name: str, code: str, quantity: int,
    buy_price: int, sell_price: int,
    profit_pct: float, profit_won: int, reason: str,
) -> None:
...
def notify_k2_plus_alert(
    ...
    label = _K2_PLUS_LABELS.get(alert_type, alert_type)
    lines = [
        f"🔷 <b>K2플러스 [{label}]</b>",
        f"종목: {name} ({code})",
        f"현재가: {current:,}원" if current > 0 else "",
        f"세력봉일: {entry.get('power_date', '-')} (D{entry.get('day_num', '?')})",
        f"K1 {entry.get('k1', 0):,} | K2 {entry.get('k2', 0):,} | "
        f"고점 {entry.get('fib_high', 0):,} | 저점 {entry.get('fib_low', 0):,}",
        f"💡 {message}",
        "⚠️ 시뮬만 — 실제 주문 없음",
...
def notify_strong_v_sim_buy(
    ...
    send(
        ...
        f"수량: {quantity}주 / 가격: {price:,}원\n"
        f"사유: {reason}\n"
        f"⚠️ 시뮬만 — 실제 주문 없음 / V자·MA5 완화 필터"
    )


def notify_strong_v_sim_sell(
    name: str, code: str, quantity: int,
    buy_price: int, sell_price: int,
    profit_pct: float, profit_won: int, reason: str,
) -> None:
...
def notify_crash_bounce_sim_buy(
    name: str, code: str, quantity: int, price: int, reason: str,
) -> None:
    send(
        f"🔶🔶 <b>[낙폭반등 시뮬] 매수</b>\n"
        ...
        f"수량: {quantity}주 / 가격: {price:,}원\n"
        f"사유: {reason}\n"
        f"⚠️ 시뮬만 — 실제 주문 없음 / 빠른손절 미적용"
    )


def notify_crash_bounce_sim_sell(
    name: str, code: str, quantity: int,
    buy_price: int, sell_price: int,
    profit_pct: float, profit_won: int, reason: str,
) -> None:
...
def notify_synopex_sim_buy(
    ...
    send(
        ...
        f"수량: {quantity}주 / 가격: {price:,}원\n"
        f"사유: {reason}\n"
        f"⚠️ 시뮬만 — 실제 주문 없음 / S거래량급증 · B10단기돌파"
    )


def notify_synopex_sim_sell(
    name: str, code: str, quantity: int,
    buy_price: int, sell_price: int,
    profit_pct: float, profit_won: int, reason: str,
) -> None:
